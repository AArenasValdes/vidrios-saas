...
import {
  getLatamCountryOption,
  LATAM_COUNTRY_OPTIONS,
  type LatamCountryCode,
} from "@/utils/latam-mobile-phone";
...
export function LandingContactSection() {
  ...
  const [countryCode, setCountryCode] = useState<LatamCountryCode>("CL");
  ...
  const phonePlaceholder = useMemo(
    () => getLatamCountryOption(countryCode).placeholder,
    [countryCode]
  );
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setFeedback({
        kind: "success",
        message:
          "Listo. Te escribimos a este WhatsApp. Si quieres partir ahora, crea tu cuenta.",
      });
    } catch (error) {
      setFeedback({
        kind: "error",
          ...
          <form className={s.form} onSubmit={handleSubmit}>
            ...
            <label className={s.field}>
              ...
              <select
                ...
                value={countryCode}
                onChange={(event) =>
                  setCountryCode(event.target.value as LatamCountryCode)
                }
                onFocus={trackFormStart}
                required
              >
                {LATAM_COUNTRY_OPTIONS.map((preset) => (
