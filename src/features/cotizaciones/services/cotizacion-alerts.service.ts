...
export type CotizacionAlertKind = "aprobada" | "rechazada" | "vista" | "seguimiento";
...
function buildAlertFromRecord(
  ...
  if (record.clienteVioEn && !record.clienteRespondioEn) {
    const hoursSinceView = (now.getTime() - new Date(record.clienteVioEn).getTime()) / (1000 * 60 * 60);
    ...
    if (hoursSinceView >= 48) {
      return {
        ...
        message: `${record.clienteNombre} vio ${record.codigo} hace mas de 2 dias. ¿Hablamos?`,
