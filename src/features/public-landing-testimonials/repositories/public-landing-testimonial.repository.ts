...
type PublicLandingTestimonialRepositoryDeps = {
  clientFactory?: ReturnType<typeof createClient>;
};

type PublicLandingTestimonialRow = {
  id: EntityId;
...
function mapRow(row: PublicLandingTestimonialRow): PublicLandingTestimonial {
  return {
    id: row.id,
...
export function createPublicLandingTestimonialRepository(
  deps: PublicLandingTestimonialRepositoryDeps = {}
) {
  ...
  return {
    async listByOrganizationId(organizationId: EntityId) {
      ...
      return ((data as PublicLandingTestimonialRow[] | null) ?? []).map(mapRow);
    },

    async create(input: CreatePublicLandingTestimonialInput) {
      const { data, error } = await supabase
        .from(TABLE_NAME as never)
        .insert({
      ...
      return mapRow(data as PublicLandingTestimonialRow);
    },
...
export type PublicLandingTestimonialRepository = ReturnType<
  typeof createPublicLandingTestimonialRepository
>;

let defaultRepository: PublicLandingTestimonialRepository | null = null;

function getDefaultRepository() {
  if (!defaultRepository) {
    defaultRepository = createPublicLandingTestimonialRepository();
  }

  return defaultRepository;
}

export const publicLandingTestimonialRepository: PublicLandingTestimonialRepository = {
  listByOrganizationId(...args) {
    return getDefaultRepository().listByOrganizationId(...args);
  },
  create(...args) {
    return getDefaultRepository().create(...args);
  },
  update(...args) {
    return getDefaultRepository().update(...args);
  },
};
