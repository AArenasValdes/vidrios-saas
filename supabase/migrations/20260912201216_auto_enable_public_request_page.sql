-- El alta debe dejar disponible la pagina publica con los datos comerciales
-- minimos del registro. El usuario puede editarla u ocultarla despues.
...
begin
  ...
  -- Mismo criterio de slug que la app: minúsculas, sin tildes y con guiones.
  -- Si el nombre ya existe, el id de organización evita colisiones.
  v_public_slug := translate(lower(btrim(v_empresa_nombre)), 'áéíóúüñ', 'aeiouun');
  v_public_slug := regexp_replace(v_public_slug, '[^a-z0-9]+', '-', 'g');
  v_public_slug := regexp_replace(v_public_slug, '(^-+|-+$)', '', 'g');
  v_public_slug := left(v_public_slug, 48);

  if v_public_slug = '' then
    v_public_slug := 'mi-empresa';
  end if;

  perform pg_advisory_xact_lock(
    hashtextextended('public-request-slug:' || v_public_slug, 0)
  );

  if exists (
    select 1
    from public.organization_profile as profile
    where lower(profile.solicitud_publica_slug) = lower(v_public_slug)
      and profile.organization_id <> v_organization_id
  ) then
    v_public_slug_suffix := '-' || v_organization_id::text;
    v_public_slug := left(
      v_public_slug,
      48 - length(v_public_slug_suffix)
    ) || v_public_slug_suffix;
  end if;
  ...
  insert into public.organization_profile (
    ...
    public_name, public_zone, solicitud_publica_slug, is_published,
    ...
    v_empresa_nombre, nullif(v_ciudad_comuna, ''), v_public_slug, true,
    v_country_code, v_currency_code, v_locale, v_timezone,
    ...
    solicitud_publica_slug = case when nullif(btrim(public.organization_profile.solicitud_publica_slug), '') is null then excluded.solicitud_publica_slug else public.organization_profile.solicitud_publica_slug end,
    is_published = case when nullif(btrim(public.organization_profile.solicitud_publica_slug), '') is null then true else public.organization_profile.is_published end,
...
-- Repara cuentas creadas antes de esta migración que quedaron sin slug.
-- Solo toca perfiles sin página identificable; páginas ya configuradas conservan
-- su slug y estado de publicación.
do $$
declare
  profile record;
  base_slug text;
...
begin
  for profile in
    ...
    where nullif(btrim(solicitud_publica_slug), '') is null
  loop
    base_slug := translate(lower(btrim(coalesce(profile.empresa_nombre, ''))), 'áéíóúüñ', 'aeiouun');
    base_slug := regexp_replace(base_slug, '[^a-z0-9]+', '-', 'g');
    base_slug := regexp_replace(base_slug, '(^-+|-+$)', '', 'g');

    if base_slug = '' then
      base_slug := 'mi-empresa';
    end if;

    suffix := '-' || profile.organization_id::text;
    base_slug := left(base_slug, 48 - length(suffix)) || suffix;

    update public.organization_profile
    set solicitud_publica_slug = base_slug,
        is_published = true,
        actualizado_en = timezone('utc', now())
