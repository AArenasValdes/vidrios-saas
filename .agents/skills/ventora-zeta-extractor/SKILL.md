...
**Navegación principal:** Codex Browser (`references/NAVIGATION.md`).  
...
- Auth: reutilizar sesión del taller en Codex Browser; Playwright auth solo en `tmp/zeta-auth/` (gitignored).
...
## Patrón Codex Browser: state → interact → state
...
2. Codex Browser: proyecto nuevo. Un marco/configuración.
...
- Codex Browser = navegación; scripts PNPM = verdad auditable.
...
Agente: lee coverage.json y targets.json → SKIP confirmed → navega con Codex Browser → raw → ingest --limit=1
```

```text
Usuario: usa ventora-zeta-extractor para L-25 DVH PIERNA ABIERTA 2H
Agente: Codex Browser → proyecto nuevo → TE4104 → Plan de armado → raw → ingest → confirmed solo si valida
```
