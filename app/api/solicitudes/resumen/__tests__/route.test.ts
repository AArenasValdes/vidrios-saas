...
jest.mock("@/features/subscriptions/services/subscription-route-access.service", () => ({
  ...
  resolveSolicitudesManagementAccess: jest.fn(),
}));
...
import { resolveSolicitudesManagementAccess } from "@/features/subscriptions/services/subscription-route-access.service";
...
describe("/api/solicitudes/resumen", () => {
  ...
  it("usa el resumen global cuando el admin esta habilitado para revisar todo", async () => {
    (resolveSolicitudesManagementAccess as jest.Mock).mockResolvedValue({
