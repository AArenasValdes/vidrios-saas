...
flowchart TD
    ...
    F --> H
    G --> H
    H --> I[UTM + eventos Ventora]
    I --> J[Solicitudes y trials]
    J --> K[Reporte y nuevo experimento]
```
...
2. **Fase B — Onboarding automático medible:** implementada y registrada en remoto mediante `growth_onboarding_videos` y `growth_onboarding_events`. El founder configura una vez un video base para celular y otro para computador en `/admin/marketing/onboarding`; toda cuenta nueva recibe automáticamente el que corresponde al entrar a `/activacion`. Las asignaciones por empresa quedan sólo como override de piloto compatible, fuera de la operación normal. Los eventos de primera cotización y PDF se capturan desde PostgreSQL, no desde el navegador. Falta cargar y validar los dos videos reales antes de escalar.
