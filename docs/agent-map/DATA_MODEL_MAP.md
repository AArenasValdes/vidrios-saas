...
- **Metadata L25 SODAL (2026-09-18)**: en `observaciones` via `encodeCotizacionItemPresentationMeta` — tokens `[lck:]` (catalog key), `[fgl:]` (glazing monolithic|dvh), `[flg:]` (pierna open|closed), `[frf:]` (refuerzo normal|reinforced). Inferencia automática de hojas y glazing; usuario elige pierna/refuerzo. Resolver `resolveFabricationRecipe()` matchea identidad completa contra recetas Zeta `validated`.
...
- **Campos importantes**: `id` (bigint PK), `organization_id` (FK), `nombre`, `categoria` (CHECK: aluminio/pvc/vidrio/shower/accesorios/otros), `unidad_cobro` (CHECK: m2/metro_lineal/unidad/valor_manual), `material` (Aluminio/PVC/Cristal), `costo_base`, `precio_m2_sugerido`, `minimo_cobrable`, `redondeo_precio` (DEFAULT 1000), `merma_pct`, `margen_objetivo_pct`, `proveedor`, `vigencia_desde`, `vigencia_hasta`, `catalog_metadata` (jsonb; para `categoria='vidrio'` guarda `espesor`, `terminacion` y opcionalmente `glassSheetOptimization` con `version`, `enabled`, `widthMm`, `heightMm`, `costClp`, `billingRule`), `vidrio_principal_recomendado`, `is_active`, `sort_order`, `creado_en`, `actualizado_en`, `eliminado_en`
- **Metadata comercial de vidrio (2026-09-18)**: `catalog_metadata.glassSheetOptimization` guarda la configuración opcional de plancha sin migración de schema; `merma_pct` sigue siendo la merma propia de la línea. La V1 calcula consumo/fracción comercial por superficie y no representa nesting 2D ni inventario de retazos.
...
- **Estado**: Implementada en remoto con `20260729230407_fabrication_recipes_persistence`, grants `20260730001306_harden_fabrication_recipe_grants`, metadatos Fase 4 `20260730003756_fabrication_recipe_validation_metadata`, procedencia explícita `20260914120000_fabrication_recipe_provenance` y evidencia P1 `20260914142228_p1_workshop_recipe_evidence`. Esas migraciones fueron aplicadas y verificadas remotamente. La corrección local `20260914170000_l42_normal_double_junquillo.sql` aún está pendiente de aplicación remota. La UI guiada incorpora asistente de texto solo para producir borradores JSON.
...
- **Campos importantes**: `id` (uuid PK), `organization_id` (bigint nullable), `line_template_id` (bigint nullable FK a `cotizacion_line_templates`), `scope` (`ventora|organization`), `provider_name`, `line_name`, `typology`, `leaves_count`, `variant`, `version`, `status` (`draft|testing|validated|review_required|archived`), `definition` (jsonb validado con Zod antes de guardar), `source_type` (`manual|copied|imported_ai|legacy|workshop|manufacturer|supplier|ventora_reference|unknown`), `source_reference`, `source_name`, `source_revision`, `parent_recipe_id`, `validated_at`, `validated_by` (auth.users), `created_at`, `updated_at`, `eliminado_en`.
- **Reglas**: `scope='ventora'` exige `organization_id IS NULL`; `scope='organization'` exige `organization_id`. Una receta validada no se modifica directamente: cambios deben crear una nueva version privada con `parent_recipe_id`. Al pasar a `validated`, se exige `validated_at` y, para sesiones autenticadas, `validated_by = auth.uid()`. **Multi-variante (2026-09-17)**: varias filas activas pueden compartir el mismo `line_template_id` si difieren en `typology`, `leaves_count` y/o `variant`; la resolución en cotización exige coincidencia de hojas comerciales (`sheetScheme` / `fabricacionHojas`) y variante cuando hay más de una compatible. **L25 SODAL canónico (2026-09-18)**: `ventora:l25` tiene **18 recetas** `status='validated'` por org (`source_type='manufacturer'`, `source_name='SODAL'`, `source_reference='zeta:confirmed:sodal/l25/{recipeId}'`); identidad `typology=corredera`, `leaves_count=2|3|4`, `variant={glazing}_{leg}_{reinforcement}`; seed idempotente vía `seed-sodal-l25-recipes.ts`; legacy L25 sin prefijo `zeta:confirmed:` se archiva al backfill. `validated` aquí = catálogo técnico system-managed verificado contra Zeta, no “validada en taller del cliente”.
...
- **Proveniencia P0**: `provider_name` identifica proveedor/familia comercial; `source_type`, `source_name`, `source_reference` y `source_revision` identifican evidencia de la receta. Ninguna receta se rotula “validada en taller” sin `status='validated'`, `source_type='workshop'` y referencia/nombre explícitos; PL-SLA se conserva como `Refuerzo`, nunca como perfil PVC inventado.
- **Evidencia P1**: `source_type='workshop'` puede representar procedencia declarada sin validar toda la receta. La evidencia por regla se conserva en `definition.observaciones` y `definition.notasValidacion`; AL-32 confirma 3201/3202 (-21 mm en hoja) y conserva 3204/3205 pendientes; AL-42 confirma 4201/4202 con -17 mm en hoja y conserva 4229 pendiente de evidencia física. Una receta parcial queda `review_required` o `draft`, nunca `workshop_validated` por existencia previa.
- **Composición segura P0**: `compositionComplete` debe ser verdadero antes de presentar una receta como lista para probar, ejecutar una prueba/validación o generar un snapshot técnico. Las referencias opcionales no cuentan como reglas activas ni cortes. Para Serie 42 normal, la migración local completa el par estructural de junquillos `4229` sin alterar descuentos, códigos ni cantidades.
...
- **Campos importantes**: `organization_id` (PK, FK ON DELETE CASCADE), `empresa_nombre`, `empresa_logo_url`, `responsable_comercial`, `empresa_direccion`, `empresa_telefono`, `empresa_email`, `brand_color` (DEFAULT '#1a3a5c'), `forma_pago`, `validez_predeterminada` (DEFAULT '15 dias'), `condiciones_venta_predeterminadas`, `terminos_condiciones_predeterminados` (plantillas comerciales heredadas al crear cotizaciones; migración `20260915100000`), `mostrar_iva_en_pdf` (DEFAULT true; preferencia de empresa para mostrar desglose de IVA en PDF; migración `20260915113000`), `proveedor_preferido`, `modo_precio_preferido` (DEFAULT 'margen'), `margen_defecto` (DEFAULT 100), `unidad_medidas` (DEFAULT 'mm', CHECK mm/cm; solo visualizacion comercial de ancho/alto), `solicitud_publica_slug` (UNIQUE partial), `solicitud_publica_descripcion_corta`, `solicitud_publica_valor`, `solicitud_publica_mensaje_confianza`, `solicitud_publica_privacidad`, `solicitud_publica_horario_desde`, `solicitud_publica_horario_hasta`, `solicitud_publica_dias_atencion`, `solicitud_publica_horario_por_dia` (jsonb), `public_name`, `public_subtitle`, `public_zone`, `public_business_type`, `secondary_color` (DEFAULT '#25d366'), `hero_mode` (CHECK: image/gradient), `hero_image_url`, `hero_title`, `hero_subtitle`, `show_gallery`, `show_schedule`, `show_rating`, `rating_label`, `jobs_count_label`, `form_title`, `form_subtitle`, `is_published`, `subscription_status`, `trial_started_at`, `trial_ends_at`, `subscription_started_at`, `subscription_ends_at`, `plan_type`, `billing_period`, `payment_method`, `last_payment_at`, `founder_price_locked`
...
| `growth_content_items` | Cola editorial interna: pieza, guion, CTA, estado, revisión humana de claim, UTM de cuatro parámetros y metadata manual de distribución en grupos |
...
## Condiciones comerciales y visibilidad de IVA en PDF (2026-09-15)

- `20260915100000_quote_default_commercial_conditions.sql` agrega plantillas en `organization_profile` (`validez_predeterminada`, `condiciones_venta_predeterminadas`, `terminos_condiciones_predeterminados`) y snapshot por cotización (`condiciones_de_pago`, `condiciones_venta`, `terminos_condiciones`). Separa textos comerciales del cliente de `notas` internas.
- `20260915113000_mostrar_iva_en_pdf.sql` agrega `mostrar_iva_en_pdf` en empresa (NOT NULL, default `true`) y override nullable por cotización. Solo controla presentación del desglose tributario en PDF/vistas cliente; no altera `iva`, `total`, descuento, flete ni redondeo.
- No hay tablas, policies RLS ni índices nuevos. Los grants columnares de `organization_profile` para `authenticated` se reemiten en ambas migraciones.
- Compatibilidad: `cotizaciones.mostrar_iva_en_pdf IS NULL` se interpreta como mostrar desglose. Helper: `resolveMostrarIvaEnPdf()` en `quote-pdf-display.service.ts`.
