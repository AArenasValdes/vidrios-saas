...
- **Fase actual = Fase 4 — módulo técnico esencial** (2026-09-21). Fuente nueva: `fabrication_recipes` + `fabrication_recipe_tests`, administrador guiado por línea, laboratorio y selector de receta validada en cotización; snapshot formal en `cotizacion_items.fabricacion_snapshot`. Catálogo comercial **31 líneas** con auditorías de integridad y fabricación. Veratec 7400 (PVC, Alumétrica) suma tres variantes documentales: mono 4 mm lista para probar; TP 20/24 mm bloqueadas por discrepancias de ficha. Línea 45 tiene destajes Sodal/Indalum (`lista_para_validar`). Serie 4800 tiene destajes SODAL Diamond (`lista_para_validar`). Ninguna es validación de taller. El P0 de consistencia exige `compositionComplete` antes de “Lista para probar” o snapshot técnico. `fabricationRecipePack`, `fabricationRecipe` y `[cub:]` quedan solo como lectura/compatibilidad. Print interno: `/print/cotizaciones/[id]/fabricacion`.
