...
|---|---|---|
| `/print/cotizaciones/[id]/fabricacion` | Resumen interno de fabricación / pauta (sin precios). Separado del PDF cliente. Desktop: bloques colapsables Cubicación/Despiece/Pauta y modal de despiece. Mobile: lista de componentes + detalle (Resumen/Cortes/Despiece) y consolidado, sin acordeón. | `app/print/cotizaciones/[id]/fabricacion/page.tsx`, `fabricacion-resumen-view.tsx`, `fabricacion-resumen-movil.tsx`, `src/features/cotizaciones/line-templates/types/fabrication-quote-summary.ts` |
| `/print/cotizaciones/[id]` | Visor, impresión y descarga PDF. Descarga rasteriza la misma hoja visible que usa la impresión en escritorio, eliminando el árbol de exportación oculto que podía divergir. En móvil, “Imprimir” prepara/abre el PDF para continuar desde el visor del navegador. Dibujo 25/50/25 se mantiene para el título exacto “Ventana corredera 3 hojas, 2 móviles + 1 fija”; caché con revisión de renderer. Características vía `buildCotizacionItemPrintSpecs()` (sin Material/Color en Espejo/Cubierta de mesa). | `app/print/cotizaciones/[id]/page.tsx`, `app/print/cotizaciones/[id]/_utils/item-print-specs.ts`, `src/features/cotizaciones/visual-composer/` |
