...
- **Archivos principales**:
  ...
  - `src/features/cotizaciones/line-templates/services/default-line-catalog.ts` (catálogo Ventora **30 líneas** canónicas + `seedDefaultLineCatalog` idempotente por `catalog_key`)
...
- **Fase 4 catálogo e integridad (actualizado 2026-09-20)**: catálogo canónico **30 líneas**; AL-32/AL-42 son proyectantes con códigos precargados y descuentos pendientes. P2U añade AM-35, Línea 15, Línea 4000 y Línea 12 como familias tradicionales/multiproveedor con procedencia documental y pauta incompleta donde corresponde. **Línea 45** entra con destajes Serie 45 practicable (`serie-45-practicable-recipe.ts`): vidrio `X−170`/`Y−183`, marco 4522, bastidor 4531, junquillo 4534; no se infieren descuentos. **Serie 4800** entra con destajes SODAL Diamond (`serie-4800-corredera-recipe.ts`): vidrio `X/2−44`/`Y−93`, rieles `X−16`, jamba `Y`, zócalo/cabezal `X/2−15`, traslapo/pierna `Y−32`; variantes normal (4806/4808) y reforzada (4810/4811). No es validación de taller. `reparar-borrador-proyectante.service.ts` reconoce precargas genéricas intactas; su repositorio las corrige por organización antes de cargar recetas, preservando ajustes y pruebas. El catálogo ya no muestra “Empieza más rápido” ni su base corredera duplicada. Precio m² automático al asignar línea; gate **Probar** sin bloqueo por vidrio base; `RecipeGlassNamePicker` en editor de receta; UX hero/tira comercial simplificada en `recipe-guided-editor`.
