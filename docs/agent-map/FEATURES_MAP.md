...
- **Fase 4 catálogo e integridad (actualizado 2026-09-13)**: catálogo canónico **25 líneas**; AL-32/AL-42 son proyectantes con códigos precargados y descuentos pendientes. `reparar-borrador-proyectante.service.ts` reconoce precargas genéricas intactas; su repositorio las corrige por organización antes de cargar recetas, preservando ajustes y pruebas. El catálogo ya no muestra “Empieza más rápido” ni su base corredera duplicada. Precio m² automático al asignar línea; gate **Probar** sin bloqueo por vidrio base; `RecipeGlassNamePicker` en editor de receta; UX hero/tira comercial simplificada en `recipe-guided-editor`.
