...
## Feature: Ideas y comentarios de mejora

- **Que hace**: Permite que los talleres compartan una propuesta por área y agreguen un comentario opcional; el equipo de Ventora ve menciones, talleres por categoria y prioriza cada aporte.
- **Rutas involucradas**: `/sugerencias`, `/api/sugerencias`, `/admin/sugerencias`, `/api/admin/sugerencias`, `/api/admin/sugerencias/[id]`
- **Archivos principales**:
  - `src/features/product-feedback/types/product-feedback.ts`
  - `src/features/product-feedback/components/product-feedback-form.tsx`
  - `src/features/product-feedback/hooks/use-product-feedback.ts`
  - `src/features/product-feedback/services/product-feedback.service.ts`
  - `src/features/product-feedback/repositories/product-feedback.repository.ts`
  - `src/features/product-feedback/components/admin-product-feedback-workspace.tsx`
  - `src/features/product-feedback/services/admin-product-feedback.service.ts`
  - `src/components/layout/app-shell.tsx` (entrada en menu de cuenta)
  - `src/features/admin/config/admin-nav.config.ts`
  - `supabase/migrations/*_product_feedback.sql`
- **Tablas Supabase**: `product_feedback` (migracion local preparada; pendiente de aplicar remotamente)
- **Flujo de datos**: Categoria/comentario -> `/api/sugerencias` valida sesion y deriva `organization_id` del perfil -> RLS permite INSERT solo a auth.uid() en su organizacion -> panel founder protegido por allowlist agrega menciones/talleres y actualiza estado desde servidor.
- **Acceso y privacidad**: El taller no puede leer ni modificar sugerencias; las ideas de otros talleres no se muestran en Ventora cliente. Lectura global solo desde servidor despues de `resolveVentoraAdminRouteContext()`.
- **Donde editar UI**: formulario en `src/features/product-feedback/components/product-feedback-form.tsx`; tracker en `src/features/product-feedback/components/admin-product-feedback-workspace.tsx`.
- **Riesgos al modificar**: Mantener RLS de insercion por `organization_id + auth_user_id`; nunca exponer `service_role` ni respuestas de otros talleres al cliente.

## Feature: Novedades de Ventora

- **Qué hace**: Permite comunicar mejoras y funciones nuevas a los talleres, asociando cada anuncio con una propuesta ya implementada si corresponde.
- **Rutas involucradas**: `/novedades`, `/api/novedades`, `/admin/novedades`, `/api/admin/novedades`, `/api/admin/novedades/[id]`
- **Archivos principales**:
  - `src/features/product-announcements/types/product-announcement.ts`
  - `src/features/product-announcements/services/product-announcement.service.ts`
  - `src/features/product-announcements/components/product-announcements-feed.tsx`
  - `src/features/product-announcements/components/admin-product-announcements-workspace.tsx`
  - `src/components/layout/app-shell.tsx` (acceso desde notificaciones y contador)
  - `src/features/admin/config/admin-nav.config.ts`
  - `supabase/migrations/*_product_announcements.sql`
- **Tablas Supabase**: `product_announcements`, `product_announcement_reads`; relación opcional con `product_feedback`.
  - **Flujo**: Panel founder guarda como borrador -> publicación explícita -> el contador de la campana incluye anuncios publicados sin leer -> en `/novedades`, tocar **Ver actualización** abre el detalle y registra la lectura por `auth_user_id + organization_id`.
- **Privacidad**: RLS activa y grants de tablas solo a `service_role`; las APIs exigen sesión de taller o founder allowlist y derivan el tenant desde el perfil.
- **Decisión de producto**: La entrada vive separada de la bandeja de alertas comerciales. No se envían push ni correo en este MVP.
- **Riesgos**: No publicar automáticamente al cambiar una propuesta a implementada; no permitir enlaces externos desde el CTA.
...
- **Archivos principales**:
  ...
  - `app/(pwa-app)/cotizaciones/nueva/_components/paso-dos/quote-studio-financial-panel.tsx` (Quote Studio desktop ≥1024; copy V2 recargo vs margen real; editor de costos internos)
  ...
  - `src/features/cotizaciones/services/quote-studio-financial.service.ts` (Costos y rentabilidad V1: `materialCostSource`, `isProfitabilityComplete`, `cost_basis_status`, override manual total, snapshot histórico read-only)
  - `src/features/cotizaciones/services/quote-profitability-defaults.service.ts` (defaults org nullable + `inheritedDefaultsSnapshot` + chip Valores predeterminados/Personalizado)
  - `app/(pwa-app)/cotizaciones/_components/quote-profitability-summary.tsx` (resumen interno compacto desktop paso 2/3 y detalle privado; en móvil Paso 3 se reemplaza por el acordeón de costos)
  - `app/(pwa-app)/cotizaciones/nueva/_components/paso-tres-costos-rentabilidad-movil.tsx` (acordeón móvil Paso 3: costos + rentabilidad global)
  ...
  - `src/features/cotizaciones/line-templates/services/default-line-catalog.ts` (catálogo Ventora **31 líneas** canónicas + `seedDefaultLineCatalog` idempotente por `catalog_key`)
...
- **Componentes principales**: `CotizacionMobileCard`, `CotizacionesMobileSummary`, `CotizacionesFilterFields`, `CotizacionDetalleDesktopView`, `CotizacionDetalleMobileView`, `PasoDosModoCotizacion`, `PasoDosItemLibreForm`, `PasoDosWizardFooterMovil`, `PasoDosWizardPrecioMovil`, `PasoTresCostosRentabilidadMovil`
...
- Snapshot regional: al crear, `cotizacionesAppService` consulta `organization_profile` y persiste `cotizaciones.regional_snapshot`; al editar lo preserva. PDF, vista/enlace publico y WhatsApp usan ese snapshot; historicas sin valor quedan en fallback Chile.
  - Snapshot financiero V1: `cotizacionesAppService.saveWorkflow()` -> `buildQuoteStudioFinancialSummary()` -> `cotizaciones.costo_total`, `cotizaciones.margen_pct`, `cotizaciones.utilidad_total`, `cotizaciones.cost_basis_status`, `cotizaciones.costo_materiales_manual` (override total), defaults org en `organization_profile.*_defecto`
...
- **Activo**: catalogo privado (`cotizacion_line_templates`) con import Excel/CSV/PDF tecnico. **Fase 2A/2B cerradas**. UI ficha: `lineas-precios-page-client.tsx` + wizard `line-template-form-wizard.tsx`. La tarjeta mantiene CTA estable **Configurar línea**, precio separado en `LinePriceEditor` y una única fila técnica hacia Fabricación. Constructor visual **V2**.
...
- **Consideraciones UX**: Paginas muy grandes (1000+ lineas). Workflow state persistido en sessionStorage. En desktop, Paso 1 integra el metodo de presupuesto y Paso 2 ofrece **Presupuesto** + **Constructor** sobre los mismos items; el Constructor no crea otra persistencia. Paso 2 soporta dos modos de pricing: `por_item` (cada item lleva su precio, incluyendo productos de cristal por m2 desde `cotizacion_line_templates`) y `total_global` (items descriptivos, total final en Paso 3). Mobile mantiene su wizard existente. **Costos y rentabilidad V1 (2026-09-20)**: capa comercial separada de capa técnica futura. Taxonomías ortogonales: `cost_basis_status` (solo completitud material), `materialCostSource` (runtime), `isProfitabilityComplete` (reemplaza `hasCostBasis`), `profitabilityParametersOrigin` (defaults vs personalizado). MO/traslado/otros **no** completan rentabilidad solos; merma = comercial estimada global, no técnica. Override `costo_materiales_manual` reemplaza total material (anti-doble-conteo). Históricas: snapshot DB es fuente de verdad (`buildQuoteStudioFinancialSummaryFromPersistedSnapshot`). **Futuro capa 3**: `materialCostSource=calculated_technical` debe forzar merma comercial 0 (desperdicio ya incluido); no implementar en V1. En **móvil**, Paso 2 solo define precio de pieza; Paso 3 concentra costos/rentabilidad con acordeón, CTA precio recomendado solo si `isProfitabilityComplete`. Desktop conserva Quote Studio ≥1024. Copy incompleta: “Rentabilidad incompleta” / “Rentabilidad pendiente”. No se expone en PDF, WhatsApp ni link público. Item libre (`tipoItem = "item_libre_con_valor"`) no requiere linea, vidrio, color, sistema, configuracion, medidas ni croquis. El quick edit (edicion rapida) ignora items libres. Si la cuenta esta vencida, el listado sigue visible pero crear/editar/eliminar deben quedar bloqueados. **No interrumpir al maestro post-PDF**: descarga registra actividad en silencio; marcar aprobada/rechazada/terminada queda en detalle o menu secundario. **Componentes solo vidrio** (`Espejo`, `Cubierta de mesa`) y productos de catalogo `categoria='vidrio'`: no pedir Aluminio/PVC ni color de perfil; en Espejo mostrar seccion **Espejos** con recomendados 3-6 mm; el resto del catalogo (ventanas, puertas, etc.) sigue pidiendo material y color como antes.
...
- **Fase 4 catálogo e integridad (actualizado 2026-09-21)**: catálogo canónico **31 líneas**; AL-32/AL-42 son proyectantes con códigos precargados y descuentos pendientes. P2U añade AM-35, Línea 15, Línea 4000 y Línea 12 como familias tradicionales/multiproveedor con procedencia documental y pauta incompleta donde corresponde. **Veratec 7400** (`veratec-7400-corredera-recipe.ts`) integra 12 destajes, vidrio `X/2−158` / `Y−177`, perfiles de barra documentada 5.800 mm y 13 consumos de herrajes; 20 mm queda pendiente por diferencia de tipología y 24 mm por el conflicto 6307/7063. **Línea 45** entra con destajes Serie 45 practicable (`serie-45-practicable-recipe.ts`): vidrio `X−170`/`Y−183`, marco 4522, bastidor 4531, junquillo 4534; no se infieren descuentos. **Serie 4800** entra con destajes SODAL Diamond (`serie-4800-corredera-recipe.ts`): vidrio `X/2−44`/`Y−93`, rieles `X−16`, jamba `Y`, zócalo/cabezal `X/2−15`, traslapo/pierna `Y−32`; variantes normal (4806/4808) y reforzada (4810/4811). Ninguna de esas pautas está validada en taller. `reparar-borrador-proyectante.service.ts` reconoce precargas genéricas intactas; su repositorio las corrige por organización antes de cargar recetas, preservando ajustes y pruebas. El catálogo ya no muestra “Empieza más rápido” ni su base corredera duplicada. Precio m² automático al asignar línea; gate **Probar** sin bloqueo por vidrio base; `RecipeGlassNamePicker` en editor de receta; UX hero/tira comercial simplificada en `recipe-guided-editor`.
- **P1 recetas reales (2026-09-14)**: migración remota idempotente registra procedencia `workshop` para Serie 5000/20/25 sin alterar sus descuentos; versiona AL-32 conservando 3204/3205 y confirma solo marco 0/hoja -21; actualiza AL-42 normal a marco 0/hoja -17, conserva 4229 y deja vidrio/accesorios pendientes. La etiqueta `workshop_validated` exige evidencia completa, prueba aprobada y estado validado.
- **P2A recetas SODAL (2026-09-14)**: `sodal-p2a-recipes.ts` y `20260914153339_p2a_sodal_aluminum_recipes.sql` documentan siete líneas de aluminio con variantes independientes, procedencia `manufacturer/SODAL`, versionado padre y fórmulas lineales visibles sin convertirlas en unidades ficticias. 3200 mantiene 3221/3225 separados y 4600 mantiene quicio mecánico/hidráulico separado; todas siguen pendientes de prueba física.
...
- **Variantes por línea (2026-09-17)**: cinco bases Ventora admiten múltiples filas `fabrication_recipes` por línea comercial (`typology + leaves_count + variant`). Cotización matchea hojas comerciales reales (`sheetScheme`); L25 3H/4H reforzada · pierna abierta = slots `draft` sin fórmulas inventadas; snapshots históricos inmutables. Ver `line-base-variant-catalog.ts` y `fabricacion-line-variant.service.ts`.
...
- **Archivos principales**:
  ...
  - `src/features/fabricacion/services/fabricacion-line-variant.service.ts`
  - `src/features/cotizaciones/line-templates/services/seed-line-variant-recipes.ts`
  ...
  - `src/features/fabricacion/services/line-operational-status.service.ts` (fuente única para estado técnico, evidencia, precio y `quotable`)
...
- **Tablas Supabase**: `fabrication_recipes`, `fabrication_recipe_tests` (migracion base `20260729230407`, grants `20260730001306`, metadatos de validacion `20260730003756`, procedencia explícita `20260914120000_fabrication_recipe_provenance` y evidencia de taller `20260914142228_p1_workshop_recipe_evidence`). Esas migraciones están aplicadas y verificadas en remoto; `20260914170000_l42_normal_double_junquillo.sql` queda preparada localmente y pendiente de aplicación remota. No toca tablas legacy.
...
- **Biblioteca priorizada**: ALAR L20/L25/L5000 se copian como borradores sugeridos bloqueados hasta completar codigos, datos de barra y pruebas. SODAL Serie 20/4800/S-33/42 queda como linea reconocida; Serie 3200 ahora tiene una `definition` borrador L/ST/TP con códigos y cortes de una hoja aportados por el estudio técnico, manteniendo pendiente la validación del taller.
...
- **Corrección P0 (2026-09-14)**: catálogo, fabricación y auditorías comparten `technicalStatus` (`incomplete|calculable`), `validationStatus` (`unverified|documented|workshop_validated`), `pricingStatus` (`missing|configured`) y `quotable` (línea activa + precio válido). “Lista para probar” ya no significa validada ni cotizable. `puerta_vaiven` identifica Serie 4600 y `pvc_monorriel` identifica Andes Monorriel; no se sustituyen por corredera. La composición incompleta bloquea también la prueba, validación y snapshot técnico aunque existan reglas calculables.
- **Corrección Serie 42 normal (2026-09-14, local)**: una hoja exige el par completo de junquillos: `4229` horizontal y vertical, 2 cortes por orientación. La migración idempotente cambia únicamente `requerido=false` a `true` en el junquillo horizontal de la receta P1 de organización 39/línea 317; conserva `-90 mm`, códigos, cantidades, procedencia, estado `draft`, vidrio y accesorios pendientes. No fue subida ni aplicada remotamente.
...
- **Archivos principales**:
  ...
  - `src/features/organization-profile/types/measure-unit.ts`
  - `src/features/organization-profile/services/measure-unit.service.ts`
  - `src/features/organization-profile/hooks/use-organization-measure-unit.ts`
