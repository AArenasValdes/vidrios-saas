...
- **Archivos principales**:
  ...
  - `src/features/cotizaciones/services/quote-studio-financial.service.ts` (Costos y rentabilidad V2: recargo vs margen real, `hasCostBasis`, precio recomendado sobre venta neta)
  ...
  - `src/features/cotizaciones/line-templates/services/default-line-catalog.ts` (catálogo Ventora **29 líneas** canónicas + `seedDefaultLineCatalog` idempotente por `catalog_key`)
  ...
  - `src/features/cotizaciones/line-templates/services/auditoria-integridad-catalogo-lineas.service.ts` (clasificación primaria de integridad de códigos; suma 29)
...
- **Consideraciones UX**: Paginas muy grandes (1000+ lineas). Workflow state persistido en sessionStorage. En desktop, Paso 1 integra el metodo de presupuesto y Paso 2 ofrece **Presupuesto** + **Constructor** sobre los mismos items; el Constructor no crea otra persistencia. Paso 2 soporta dos modos de pricing: `por_item` (cada item lleva su precio, incluyendo productos de cristal por m2 desde `cotizacion_line_templates`) y `total_global` (items descriptivos, total final en Paso 3). Mobile mantiene su wizard existente. **Costos y rentabilidad V2 (2026-09-14)**: el recargo sobre costo (markup de pieza) ya no se llama “margen”; “margen real” queda reservado a utilidad/venta neta. En **móvil**, Paso 2 solo define el precio de la pieza (`Costo + recargo` / `Recargo sobre costo (%)`); Paso 3 concentra costos y rentabilidad en un acordeón bajo Flete/Descuento. Desktop conserva Quote Studio y el resumen interno compacto. Si no hay `hasCostBasis` muestra “Rentabilidad pendiente”. No se expone en PDF, WhatsApp ni link público. El editor completo de Quote Studio sigue siendo desktop ≥1024. Validar 390 px y 430 px: el compacto mobile no debe hinchar el dock ni cambiar CTA/PDF/WhatsApp. Los snapshots financieros se siguen guardando en campos existentes a nivel cotizacion (`costo_total`, `utilidad_total`, `margen_pct`). Item libre (`tipoItem = "item_libre_con_valor"`) no requiere linea, vidrio, color, sistema, configuracion, medidas ni croquis. El quick edit (edicion rapida) ignora items libres. Si la cuenta esta vencida, el listado sigue visible pero crear/editar/eliminar deben quedar bloqueados. **No interrumpir al maestro post-PDF**: descarga registra actividad en silencio; marcar aprobada/rechazada/terminada queda en detalle o menu secundario. **Componentes solo vidrio** (`Espejo`, `Cubierta de mesa`) y productos de catalogo `categoria='vidrio'`: no pedir Aluminio/PVC ni color de perfil; en Espejo mostrar seccion **Espejos** con recomendados 3-6 mm; el resto del catalogo (ventanas, puertas, etc.) sigue pidiendo material y color como antes. **Optimización de vidrio V1 (2026-09-18)**: un producto de cristal puede guardar formato de plancha, costo, merma de línea y regla comercial (`exact|quarter|half|full`) y aplicar ese consumo automáticamente al precio por m². La UI móvil muestra **Optimizado por Ventora**. Es cálculo comercial por superficie/fracción de plancha; la cantidad de una misma pieza se consolida antes de calcular la fracción. V1 no consolida entre ítems distintos ni hace nesting 2D, inventario o pauta física de corte.
...
- **Fase 4 catálogo e integridad (actualizado 2026-09-14)**: catálogo canónico **29 líneas**; AL-32/AL-42 son proyectantes con códigos precargados y descuentos pendientes. P2U añade AM-35, Línea 15, Línea 4000, Línea 45 y Línea 12 como familias tradicionales/multiproveedor con procedencia documental y pauta incompleta donde corresponde; no se infieren descuentos. `reparar-borrador-proyectante.service.ts` reconoce precargas genéricas intactas; su repositorio las corrige por organización antes de cargar recetas, preservando ajustes y pruebas. El catálogo ya no muestra “Empieza más rápido” ni su base corredera duplicada. Precio m² automático al asignar línea; gate **Probar** sin bloqueo por vidrio base; `RecipeGlassNamePicker` en editor de receta; UX hero/tira comercial simplificada en `recipe-guided-editor`.
