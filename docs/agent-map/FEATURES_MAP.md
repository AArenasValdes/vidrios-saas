...
- **Fase actual**: **Fase 4 — Cubicacion asistida y pauta revisable**. Modelo vigente (2026-07-21): **recetas de fabricación** con componentes reales (riel/jamba/cabezal/etc.), reglas guiadas y validación por componente dentro del wizard de línea. Snapshot cotización **v2** congela la receta. Partidas Marco/Hoja V1 quedan solo como migración.
- **Camino 2 histórico (2026-07-19)**: 3 partidas genéricas; **superseded** por recetas. Tipologías complejas siguen en constructor. Handoff: `docs/agent-map/CUBICACION_PAUTA_HANDOFF.md`.
- **Alcance Fase 4**: cubicacion sin precios, pauta por perfil real (o `Por asignar` hasta calibrar), snapshot v2, ajuste, consolidado, calibracion/validación por componente. No optimizador real, nesting, CAD, inventario ni fabricacion automatica.
- **Smoke 2026-07-21**: login test + flujo cotizar confirma despiece con funciones de receta (Riel/Jamba/Cabezal).
